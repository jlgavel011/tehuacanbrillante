...
export function MaintenanceStopsBySubsubsystem() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/analytics/maintenance-stops-by-subsubsystem");
        if (!response.ok) {
  ...
  if (isLoading) {
    return (
      <Card className="h-full">
        <Title>Subsubsistemas con Más Paros por Mantenimiento</Title>
  ...
  const formattedData = data.map(item => ({
    ...item,
    name: `${item.name} (${item.subsistema} - ${item.sistema} - ${item.linea})`
  }));
