...
export function ProductionHeatmapByHour() {
  ...
  const handleViewDetails = () => {
    router.push(`/reports/production-heatmap-by-hour-detail${
  ...
  return (
    <ReportCard
      title="Horas con Mayor Producción"
      subtitle="Distribución de la producción por hora del día"
      headerExtra={
        <Button
          variant="ghost"
          size="icon"
        ...
        {loading ? (
          ...
          <div className="flex flex-col space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {data.map((hour) => (
                    <div key={hour.hourIndex} className="flex items-center">
                      <div className="w-[80px] font-medium text-sm">{formatHour(hour.hour)}</div>
                      <div className="flex-1 h-8">
                        <div
                          className={`relative h-full rounded-md ${getHeatmapColor(
                            metric === "cajas" ? hour.cajasProducidas : hour.litrosProducidos,
                        ...
                        >
                          <span className="absolute left-2 top-1/2 transform -translate-y-1/2 text-xs text-gray-700 font-medium">
                            {metric === "cajas" 
                              ? hour.cajasProducidas.toLocaleString()
                              : hour.litrosProducidos.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="p-3 border-t border-border/50 bg-slate-50/50 text-center text-sm text-muted-foreground">
              El gráfico muestra la producción total por hora del día
            </div>
          </div>
        )}
      </div>
