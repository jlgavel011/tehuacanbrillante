...
export function MostProducedSizes() {
  ...
  const [boxesData, setBoxesData] = useState<SizeBoxesData[]>([]);
  const [litersData, setLitersData] = useState<SizeLitersData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch boxes data
        const boxesResponse = await fetch(`/api/analytics/most-produced-sizes?${params}`);
        ...
        // Fetch liters data
        const litersResponse = await fetch(`/api/analytics/most-produced-sizes-liters?${params}`);
