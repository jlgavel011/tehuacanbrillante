...
export function PlannedVsProducedBoxesByLineChief() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/analytics/planned-vs-produced-boxes-by-line-chief?from=${from}&to=${to}&limit=10`
        );
  ...
  const handleViewDetails = () => {
    ...
    router.push(`/reports/planned-vs-produced-boxes-by-line-chief-detail`);
  };
  ...
  return (
    <ReportCard
      title="Cajas Planificadas vs Producidas por Jefe de Línea"
      subtitle="Comparación de producción real vs planificada por jefe de línea"
      headerExtra={
        <Button
          variant="ghost"
          size="icon"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-lg bg-[#fdf4ff] p-4 text-center">
              <div className="flex items-center justify-center space-x-2 mb-1">
                ...
                <p className="text-sm text-muted-foreground">Jefes de Línea Analizados</p>
              </div>
