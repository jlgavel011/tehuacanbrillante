...
export function QualityDeviationStops() {
  const router = useRouter();
  const { date: dateRange } = useDateRange();
  const [data, setData] = useState<StopStats[]>([]);
  ...
  if (loading) {
    return (
      <ReportCard
        title="Paros por Desviaciones de Calidad"
        subtitle="Top 5 desviaciones más frecuentes"
      >
  ...
  // Tomar los top 5 para el gráfico
  ...
  const handleViewDetails = () => {
    router.push("/reports/quality-deviation-stops");
  };
