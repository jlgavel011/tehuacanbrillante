...
export function MostProducedBoxes() {
  ...
  const [boxesData, setBoxesData] = useState<BoxBoxesData[]>([]);
  const [litersData, setLitersData] = useState<BoxLitersData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch boxes data
        const boxesResponse = await fetch(`/api/analytics/most-produced-boxes?${params}`);
        ...
        // Fetch liters data
        const litersResponse = await fetch(`/api/analytics/most-produced-boxes-liters?${params}`);
