...
interface QualityDeviationData {
  name: string;
  paros: number;
  tiempo_total: number;
  porcentaje: number;
...
export default function QualityDeviationStopsReport() {
  const router = useRouter();
  const { date, selectedPeriod, setSelectedPeriod, setDate } = useDateRangeFilter();
  const [data, setData] = useState<QualityDeviationData[]>([]);
  const [filteredData, setFilteredData] = useState<QualityDeviationData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/analytics/quality-deviation-stops?${params}`);
        if (!response.ok) throw new Error("Error al cargar los datos");
        const result = await response.json();

        // Calcular el tiempo total y los porcentajes
        const tiempoTotal = result.reduce((acc: number, item: QualityDeviationData) => acc + item.tiempo_total, 0);
        const dataWithPercentages = result.map((item: QualityDeviationData) => ({
          ...item,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold">Reporte de Paros por Desviaciones de Calidad</h1>
        </div>
        <DateRangeFilter
            ...
            <>
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1">
                  <Input
                    placeholder="Buscar por desviación..."
                    value={searchTerm}
