...
export default function ModelsReport() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [boxesResponse, litersResponse] = await Promise.all([
          fetch(`/api/analytics/most-produced-models?${params}`),
          fetch(`/api/analytics/most-produced-models-liters?${params}`)
        ]);
        ...
        litersData.forEach((item: any) => {
          if (modelMap.has(item.nombre)) {
            // Update existing model
            const model = modelMap.get(item.nombre)!;
            model.litros = item.litros;
            model.porcentajeLitros = item.porcentaje;
          } else {
            // Add new model that only has liters data
            modelMap.set(item.nombre, {
              nombre: item.nombre,
              cantidad: 0,
              porcentajeCajas: 0,
  ...
  const handleDownload = () => {
    ...
    downloadCSV(csvData, "reporte_modelos");
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-semibold">Reporte de Producción por Modelo</h1>
          </div>
